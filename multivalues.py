...
def process_nodes(grainger_all, df, category):
    process_ws_df = pd.DataFrame()
    values = pd.DataFrame()

    df['Count'] =1

    temp_grainger = grainger_all.loc[grainger_all['STEP_Category_ID']==category]

    if temp_grainger.empty==False:
        print('Grainger category = ', category)
        temp_grainger = temp_grainger[['WS_SKU', 'STEP_Attr_ID', 'Grainger_Attribute_Value']]
                    
        cols = ['WS_SKU', 'STEP_Attr_ID']
    ...
    if search_level == 'single':
        for node in search_data:
            ...
            if init_ws_df.empty == False:
                ...
                init_ws_df['STEP_Attr_ID'] = init_ws_df['STEP_Attr_ID'].str.replace('_ATTR', '')
                init_ws_df['STEP_Attr_ID'] = init_ws_df['STEP_Attr_ID'].str.replace('_GATTR', '')
                init_ws_df['STEP_Attr_ID'] = init_ws_df['STEP_Attr_ID'].str.strip()
                init_ws_df['STEP_Attr_ID'] = init_ws_df['STEP_Attr_ID'].astype(int)
                
                init_ws_df['STEP_Category_ID'] = init_ws_df['STEP_Category_ID'].str.replace('_DIV1', '')
                init_ws_df['STEP_Category_ID'] = init_ws_df['STEP_Category_ID'].str.strip()
                init_ws_df['STEP_Category_ID'] = init_ws_df['STEP_Category_ID'].astype(int)
    
                cat_ids = init_ws_df['STEP_Category_ID'].unique().tolist()

                for cat in cat_ids:
                    ws_df, att_vals = process_nodes(allCATS_df, init_ws_df, cat)
                ...
                for k in range(0, len(div_list)):
                    ...
                    for j in div_list[k]:
                        ...
                        if single_df.empty==False:
                            # clean up pi_mappings data
                            single_df['STEP_Attr_ID'] = single_df['STEP_Attr_ID'].str.replace('_ATTR', '')
                            single_df['STEP_Attr_ID'] = single_df['STEP_Attr_ID'].str.replace('_GATTR', '')
                            single_df['STEP_Attr_ID'] = single_df['STEP_Attr_ID'].str.strip()
                            single_df['STEP_Attr_ID'] = single_df['STEP_Attr_ID'].astype(int)
                
                            single_df['STEP_Category_ID'] = single_df['STEP_Category_ID'].str.replace('_DIV1', '')
                            single_df['STEP_Category_ID'] = single_df['STEP_Category_ID'].str.strip()
                            single_df['STEP_Category_ID'] = single_df['STEP_Category_ID'].astype(int)
    
                            cat_ids = single_df['STEP_Category_ID'].unique().tolist()

                            for cat in cat_ids:
                                temp_node_df, temp_att_vals = process_nodes(allCATS_df, single_df, cat)
