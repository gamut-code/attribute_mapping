...
def data_out(df, node):
    ...
    df_upload = df[['WS_Node_Name', 'WS_SKU', 'WS_Attr_ID', 'WS_Attribute_Name', 'List_of_Values',\
                    '#_Values', 'Grainger_Attribute_Value', 'Potential_Replaced_Values', 'Revised Value']]
    df_upload['Count'] = 1
    df_upload = pd.DataFrame(df_upload.groupby(['WS_Node_Name', 'WS_SKU', 'WS_Attr_ID', 'WS_Attribute_Name', \
...
if data_type == 'gws_query':
    ...
    if len(search_data)>4000:
        ...
        for k  in range(0, len(div_lists)):
            ...
            init_ws_df = pd.concat([df, temp_sku_df], axis=0, sort=False) 
