...
def data_out(df, df_no_dupes, node):

    outfile = 'C:/Users/xcxg109/NonDriveFiles/'+str(node)+'_text_UOMs.xlsx'  
    writer = pd.ExcelWriter(outfile, engine='xlsxwriter')
    workbook  = writer.book

    df_no_dupes.to_excel (writer, sheet_name="Uniques", startrow=0, startcol=0, index=False)
    df.to_excel (writer, sheet_name="All Text UOMs", startrow=0, startcol=0, index=False)
    ...
    worksheet1.set_column('L:L', 50, layout)
    worksheet1.set_column('M:M', 50, layout)
    ...
    worksheet2.set_column('L:L', 50, layout)
    worksheet2.set_column('M:M', 50, layout)
    worksheet2.set_column('O:O', 50, layout)
...
if data_type == 'gws_query':
    for node in search_data:
        ...
        if df.empty == False:
            ...
            ws_df = ws_df[['Group_ID', 'WS_Category_ID', 'WS_Category_Name', 'WS_Node_Name', 'WS_SKU', \
                            'WS_Attr_ID', 'WS_Attribute_Name', 'Original_Value', 'Normalized_Value',\
                            'Potential_Replaced_Values', 'Revised Value']]

            ws_no_dupes = ws_df.drop_duplicates(subset=['WS_Attribute_Name', 'Original_Value'])
            ws_no_dupes = ws_no_dupes[['Group_ID', 'WS_Node_Name', 'WS_SKU', 'WS_Attr_ID', 'WS_Attribute_Name', \
                                       'Original_Value', 'Normalized_Value', 'Potential_Replaced_Values', \
                                       'Revised Value']]
            ws_no_dupes = ws_no_dupes.rename(columns={'WS_SKU':'Example SKU'})

            data_out(ws_df, ws_no_dupes, node)
    ...
    if df.empty == False:
        ...
        ws_df = ws_df[ws_df.Potential_Replaced_Values != '']
        ws_df = ws_df.sort_values(['Potential_Replaced_Values'], ascending=[True])
        ws_df['Group_ID'] = ws_df.groupby(ws_df['Original_Value']).grouper.group_info[0]

        ws_no_dupes = ws_df.drop_duplicates(subset=['WS_Attribute_Name', 'Original_Value'])
        ws_no_dupws = ws_no_dupes[['Group_ID', 'WS_Node_Name', 'WS_SKU', 'WS_Attr_ID', 'WS_Attribute_Name', \
                                       'Original_Value', 'Normalized_Value', 'Potential_Replaced_Values', \
                                       'Revised Value']]
