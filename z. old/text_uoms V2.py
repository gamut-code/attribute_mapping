...
def data_out(ws_df, node, batch):
    ...
    ws_no_dupes = ws_no_dupes[['Group_ID', 'WS_Node_ID', 'WS_Node_Name', 'WS_SKU', 'id', 'id_migration', \
...
#            df = gws.gws_q(gws_basic_query, 'tprod."categoryId"', node)           
        ...
        if df.empty == False:
            ...
            if len(node_ids)>130:
                num_lists = round(len(node_ids)/130, 0)
                ...
                for k in range(0, len(div_list)):
                    ...
                    for j in div_list[k]:
                        temp_df = df.loc[df['WS_Node_ID']== j]
                        temp_df['Count'] =1
                        temp_df = process_vals(temp_df)
                ...
                data_out(ws_df, ws_no_dupes, node, 'only')
                print("--- {} minutes ---".format(round((time.time() - start_time)/60, 2)))    
    ...
    if df.empty == False:
        ...
        data_out(ws_df, ws_no_dupes, 'sku', 'only')
