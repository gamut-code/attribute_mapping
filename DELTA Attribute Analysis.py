...
ws_query = """
    SELECT cat.id AS "WS Node ID"
        , cat.name as "WS Node Name"
        , tax_att.id as "WS Attribute ID"
        , tax_att.name as "WS Attribute Name"
        , tax_att."dataType" as "Data Type"
        , tax_att."multiValue" as "Multivalued?"
        , tax_att."sampleValues" as "Sample Values"
   
    FROM  taxonomy_category cat
        
    INNER JOIN taxonomy_attribute tax_att
        ON tax_att."categoryId" = cat.id
        AND tax_att.deleted = 'false'
...
def get_data_type(df, attribute):
    ...
    name_exclusions = ['Thread Size', 'Thread Depth', 'Item', 'For Use With', \
...
def grainger_process(grainger_df, grainger_all, uom_df, lov_df, lov_list, gamut_dict, grainger_node):
    ...
    if len(grainger_skus) > 7000:
        ...
        for i in range(0, num_split):
            ...
            grainger_new = pd.concat([grainger_new, temp_df], axis=0, sort=False) 
            
            print("--- grainger loop time = {} minutes ---".format(round((time.time() - loop_time)/60, 2)))
...
#            print('EMPTY DATAFRAME')
    ...
    grainger_l2 = pd.DataFrame(allCATS_df.groupby(['Family_ID'])['Count'].sum()).reset_index()
    grainger_l2 = grainger_l2['Family_ID'].tolist()
        
    print('L2s = ', grainger_l2)

    for l2 in grainger_l2:
        print('** K = ', l2)
            
        family_df = allCATS_df.loc[allCATS_df['Family_ID']== l2]
        grainger_l3 = family_df['Category_ID'].unique().tolist()

        print('Grainger L3s: ', grainger_l3)
        
        for l3 in grainger_l3:
            print('\nL3 = ', l3)
            grainger_df = family_df.loc[family_df['Category_ID']== l3]                
            ...
            if grainger_df.empty == False:
                ...
                print('{} No attribute data'.format(l3))
